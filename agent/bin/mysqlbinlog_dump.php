...
/**
 * mysqlbinlog_dump.php — 独立子进程：用 krowinski/php-mysql-replication（纯 PHP）解析 binlog，
 * 将行事件以 JSON 行输出到 stdout（每行一个结构化变更），由 agent 转发给前端 SSE。
 *
 * 用法：
 *   php bin/mysqlbinlog_dump.php --host 127.0.0.1 --port 3306 --user root \
 *       --file binlog.000014 --pos 4 [--db test] \
 *       [--start-ts 1787400000] [--end-ts 1787410000]
 *   密码经环境变量 DMS_MYSQL_PASSWORD 传入（--password 可选，缺省读 ENV）。
 *
 * 时间窗口（epoch 秒，0 = 不限）：
 *   --start-ts  早于该时刻的行事件跳过；并自动定位起点文件（SHOW BINARY LOGS 从新到旧探测
 *               各文件首个行事件 ts，取首事件 ts < startTs 的最新文件，从 pos 4 起读，
 *               经 ROTATE 续读更新文件，窗口前事件由 --start-ts 过滤）。
 *   --end-ts    行事件越过该时刻即退出；无新行事件时用心跳（本地墙钟）越过 endTs+5s 兜底退出。
 *
 * 输出行（stdout，每行一个 JSON）：
 *   {"type":"change","kind":"insert|update|delete","schema","table","columns","primaryKeys",
 *    "before","after","xid","timestamp","binlogFile","binlogPos"}
 *   {"type":"heartbeat","ts":...}（仅心跳/兜底退出用，前端忽略）
 * 错误写 stderr。exit 0 表示正常结束；非 0 表示异常。
 */
...
if ($file === '') {
    fwrite(STDERR, "mysqlbinlog_dump: --file 必填（或由 --start-ts 自动定位）\n");
    exit(2);
}
...
function emit(array $row): void
{
    ...
    if ($json === false) {
        fwrite(STDERR, 'mysqlbinlog_dump: json_encode 失败: ' . json_last_error_msg() . "\n");
        return;
    }
    fwrite(STDOUT, $json . "\n");
    flush();
    fwrite(STDERR, 'mysqlbinlog_dump: emit #' . ($row['xid'] ?? 0) . ' kind=' . ($row['kind'] ?? '?') . ' ts=' . ($row['timestamp'] ?? 0) . ' elapsed_ms=' . round($elapsed, 1) . "\n");
}
...
/**
 * 探测单个文件：从 pos 4 起消费事件流，找到第一个行事件并比较时间戳。
...
function probeOne(string $host, int $port, string $user, string $password, string $file, int $startTs): string
{
    ...
    try {
        ...
        fwrite(STDERR, 'mysqlbinlog_dump: 探测 ' . $file . ' 失败: ' . $e->getMessage() . "\n");
        return 'older';
    }
...
// ---- 起点文件定位（仅 startTs>0）----
if ($startTs > 0) {
    fwrite(STDERR, 'mysqlbinlog_dump: 按 startTs=' . $startTs . " 定位起点文件...\n");
    [$file, $pos, $why] = probeStartFile($host, $port, $user, $password, $startTs);
    if ($file === '') {
        fwrite(STDERR, "mysqlbinlog_dump: 定位失败（{$why}）\n");
        exit(2);
    }
    fwrite(STDERR, "mysqlbinlog_dump: 起点文件 {$file} pos={$pos}（{$why}）\n");
}
    ...
    new class($state) extends EventSubscribers {
        ...
        public function onXID(XidDTO $event): void
        {
            ...
            foreach ($st->pending as $row) {
                ...
                if ($st->endTs > 0 && $row['timestamp'] > 0 && $row['timestamp'] > $st->endTs) {
                    exit(0);
                }
                $row['xid'] = $xid;
                emit($row);
        ...
        public function onHeartbeat($event): void
        {
            ...
            // 兜底退出：心跳 = binlog 空闲（已追平当前文件尾）。krowinski 心跳事件的
            // eventInfo->timestamp 恒为 0（库实现如此），故用本地时间判断：
...
try {
    ...
    fwrite(STDERR, 'mysqlbinlog_dump: ' . $e->getMessage() . "\n");
    exit(1);
}
